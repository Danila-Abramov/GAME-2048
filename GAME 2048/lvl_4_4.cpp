...
void draw_text_score(SDL_Renderer*& renderer, SDL_Texture* texture)
{
	SDL_Rect rect_2 = { 1005,5, 195, 60 };
	SDL_RenderCopy(renderer, texture, NULL, &rect_2);
}

void draw_text_record(SDL_Renderer*& renderer, SDL_Texture* texture)
{
	SDL_Rect rect_3 = { 1205,85, 60, 60 };
	SDL_RenderCopy(renderer, texture, NULL, &rect_3);
}

void draw_text_record_text(SDL_Renderer*& renderer, SDL_Texture* texture)
{
...
void functions_random(int** massiv, int& rand_1)
{
	bool ok = false;
	srand(time(NULL));
	for (int i = 0; i < 4;i++)
...
void functions_lvl_4_4(SDL_Renderer* renderer, bool& quit_1, int** massiv, TTF_Font* font, int * massiv_record)
{
	...
	SDL_Rect massiv_draw = { 0,20,170,170 };
	...
	while (quit == false)
	{
		...
		if (massiv_record[0] < score)
		{
			massiv_record[0] = score;
		}

		_itoa_s(score, text, 10);
		_itoa_s(massiv_record[0], record, 10);
		textTexture = get_text_texture(renderer, text, font);
		textTexture_2 = get_text_texture(renderer, text_score, font);
		textTexture_3 = get_text_texture(renderer, record, font);
		textTexture_4 = get_text_texture(renderer, text_record, font);

		for (int i = 0; i < 4; i++)
		{
			for (int j = 0; j < 4; j++)
			{
				if (massiv[i][j] == 0) { massiv_draw.x = 300 + 170 * j; massiv_draw.y = 20 + 170 * i; SDL_RenderCopy(renderer, kletka_0, NULL, &massiv_draw); }
				if (massiv[i][j] == 2) { massiv_draw.x = 300 + 170 * j; massiv_draw.y = 20 + 170 * i; SDL_RenderCopy(renderer, kletka_2, NULL, &massiv_draw); }
				if (massiv[i][j] == 4) { massiv_draw.x = 300 + 170 * j; massiv_draw.y = 20 + 170 * i; SDL_RenderCopy(renderer, kletka_4, NULL, &massiv_draw); }
				if (massiv[i][j] == 8) { massiv_draw.x = 300 + 170 * j; massiv_draw.y = 20 + 170 * i; SDL_RenderCopy(renderer, kletka_8, NULL, &massiv_draw); }
				if (massiv[i][j] == 16) { massiv_draw.x = 300 + 170 * j; massiv_draw.y = 20 + 170 * i; SDL_RenderCopy(renderer, kletka_16, NULL, &massiv_draw); }
				if (massiv[i][j] == 32) { massiv_draw.x = 300 + 170 * j; massiv_draw.y = 20 + 170 * i; SDL_RenderCopy(renderer, kletka_32, NULL, &massiv_draw); }
				if (massiv[i][j] == 64) { massiv_draw.x = 300 + 170 * j; massiv_draw.y = 20 + 170 * i; SDL_RenderCopy(renderer, kletka_64, NULL, &massiv_draw); }
				if (massiv[i][j] == 128) { massiv_draw.x = 300 + 170 * j; massiv_draw.y = 20 + 170 * i; SDL_RenderCopy(renderer, kletka_128, NULL, &massiv_draw); }
				if (massiv[i][j] == 256) { massiv_draw.x = 300 + 170 * j; massiv_draw.y = 20 + 170 * i; SDL_RenderCopy(renderer, kletka_256, NULL, &massiv_draw); }
				if (massiv[i][j] == 512) { massiv_draw.x = 300 + 170 * j; massiv_draw.y = 20 + 170 * i; SDL_RenderCopy(renderer, kletka_512, NULL, &massiv_draw); }
				if (massiv[i][j] == 1024) { massiv_draw.x = 300 + 170 * j; massiv_draw.y = 20 + 170 * i; SDL_RenderCopy(renderer, kletka_1024, NULL, &massiv_draw); }
				if (massiv[i][j] == 2048) { massiv_draw.x = 300 + 170 * j; massiv_draw.y = 20 + 170 * i; SDL_RenderCopy(renderer, kletka_2048, NULL, &massiv_draw); }
		...
		for (int i = 0; i < 4; i++)
		{
			for (int j = 0; j <= 10; j++)
			{
				SDL_RenderDrawLine(renderer, 300, 20 + 170 + 170 * i - j, 300 + 680, 20 + 170 + 170 * i - j);
				SDL_RenderDrawLine(renderer, 300 + 170 + 170 * i + j, 20, 300 + 170 + 170 * i + j, 690);
			}
		}

		draw_text(renderer, textTexture);
		SDL_DestroyTexture(textTexture);
		draw_text_score(renderer, textTexture_2);
		SDL_DestroyTexture(textTexture_2);
		draw_text_record(renderer, textTexture_3);
		SDL_DestroyTexture(textTexture_3);
		draw_text_record_text(renderer, textTexture_4);
		...
		for (int i = 0; i < size; i++)
		{
			for (int j = 0; j < size; j++)
			{
				...
				if (massiv[i][j] == 2048) { quit = true; pobeda = true; break; }
			}
		}
		if (cnt == 16 and quit != true)
		{
			for (int i = 0; i < size; i++)
			{
			...
			if (cnt == 16 and ok1 == false and ok2 == false) { porachenie = true; quit = true;}
